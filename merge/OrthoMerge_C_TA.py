...
def distributed_TA(
    ...
    print(f"[TA] Running pure Task Arithmetic merge over distributed deltas...")

    num_tasks = len(distributed_deltas)
    assert num_tasks > 0, "distributed_deltas null"
    ...
    for i, delta in enumerate(distributed_deltas):
        assert delta.numel() == total_params, f"Task {i} error"

    merged_delta = torch.zeros(total_params, dtype=dtype, device=main_device)

    print(f"  Number of tasks: {num_tasks}")
    print(f"  Device for accumulation: {main_device}")
    print(f"  Total params: {total_params}")

    for start_idx in tqdm(range(0, total_params, chunk_size), desc="TA Accumulating"):
        end_idx = min(start_idx + chunk_size, total_params)

        chunk_sum = torch.zeros(end_idx - start_idx, dtype=dtype, device=main_device)

        for task_idx in range(num_tasks):
            delta_chunk = distributed_deltas[task_idx][start_idx:end_idx].to(
                main_device, non_blocking=True
            )
            chunk_sum += delta_chunk
            del delta_chunk

        merged_delta[start_idx:end_idx] = chunk_sum
    ...
    print("  [TA] Finished accumulating task deltas.")
    return merged_delta
...
def main():
    ...
    final_merged_delta_gpu = distributed_TA(
    ...
    final_vec = ortho_base_vec_cpu + 0.2 * final_merged_delta_gpu.cpu()
