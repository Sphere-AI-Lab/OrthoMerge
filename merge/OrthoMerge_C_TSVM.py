...
OUTPUT_PATH = "pathTo/models/Llama-3.2-3B_OrthoMerge-C-TA"
CAYLEY_CACHE_DIR = "./cache_Llama-3.2-3B_C"
    ...
    for layer_name in tqdm(target_layers, desc="Layerwise Ortho+Targets"):
        ...
        if A_list is None:
            ...
            for exp_sd in loaded_experts:
                ...
                deltas.append(w_exp - base_slice)

            stack_deltas = torch.stack(deltas, dim=0)   # [N, out, in]
            mean_delta = stack_deltas.mean(dim=0)       # [out, in]

            target_Ws = []
            for delta in deltas:
                cos_sim = torch.nn.functional.cosine_similarity(
                    delta, mean_delta, dim=1, eps=1e-6
                ).unsqueeze(1)  # [out, 1]
                conflict_mask = (cos_sim < CONFLICT_THRESHOLD).float()
                target_slice = base_slice + delta * conflict_mask
                target_Ws.append(target_slice)
            ...
            del deltas, stack_deltas, mean_delta, target_Ws, R_list, base_w
...
def distributed_TSVM(
    ...
    main_device = distributed_deltas[0].device
    dtype = distributed_deltas[0].dtype
    total_params = distributed_deltas[0].numel()

    for i, d in enumerate(distributed_deltas):
        assert d.numel() == total_params, f"Task {i} error"

    print(f"[TSV-M] Running TSV-Merge over distributed deltas ...")
    print(f"  Number of tasks: {num_tasks}")
    print(f"  Accumulation / SVD device: {main_device}")
    print(f"  Total params: {total_params}")

    distributed_deltas_cpu = [d.detach().cpu() for d in distributed_deltas]

    sv_reduction = 1.0 / num_tasks

    merged_per_key: Dict[str, Tensor] = {}
    all_keys = list(index_mgr.indices.keys())

    for key in tqdm(all_keys, desc="[TSV-M] Layer-wise merging"):
        start, end = index_mgr.indices[key]
        shape = index_mgr.shapes[key]

        is_matrix = (len(shape) == 2)
        should_svd = is_matrix and (key not in exclude_keys)

        task_tensors = [
            d[start:end].view(shape) for d in distributed_deltas_cpu
        ]
        ...
        if should_svd:
            ...
            for i, mat_cpu in enumerate(task_tensors):
                mat = mat_cpu.to(device=main_device, non_blocking=True)
                ...
                Ui_list.append(Ui[:, :reduced_rank])         # [rows, r_i]
                Si_list.append(Si[:reduced_rank])            # [r_i]
                Vi_list.append(Vhi[:reduced_rank, :])        # [r_i, cols]
            ...
            U_concat = torch.zeros(rows, sum_r, device=main_device, dtype=torch.float32)
            V_concat = torch.zeros(cols, sum_r, device=main_device, dtype=torch.float32)
            S_block = torch.zeros(sum_r, device=main_device, dtype=torch.float32)

            offset = 0
            for i in range(num_tasks):
                r_i = r_list[i]
                U_concat[:, offset:offset + r_i] = Ui_list[i]
                V_concat[:, offset:offset + r_i] = Vi_list[i].transpose(0, 1)  # [cols, r_i]
                S_block[offset:offset + r_i] = Si_list[i]
                offset += r_i

            # U_concat = P_U D_U Q_U^T -> U_hat = P_U Q_U^T
            PU, DU, QUt = torch.linalg.svd(U_concat, full_matrices=False)
            U_hat = PU @ QUt  # [rows, sum_r]

            # V_concat = P_V D_V Q_V^T -> V_hat = P_V Q_V^T
            PV, DV, QVt = torch.linalg.svd(V_concat, full_matrices=False)
            V_hat = PV @ QVt  # [cols, sum_r]

            US = U_hat * S_block.unsqueeze(0)        # [rows, sum_r]
            merged_layer = US @ V_hat.transpose(0, 1)  # [rows, cols]

            merged_per_key[key] = merged_layer.to(dtype=dtype, device="cpu")

            del Ui_list, Si_list, Vi_list, U_concat, V_concat, S_block
            del PU, DU, QUt, PV, DV, QVt, U_hat, V_hat, US, merged_layer
            torch.cuda.empty_cache()

        else:
            acc = None
            for i, mat_cpu in enumerate(task_tensors):
                vec = mat_cpu.to(device=main_device, non_blocking=True)
                if acc is None:
                    acc = vec.clone()
                else:
                    acc += (vec - acc) / (i + 1)

            merged_per_key[key] = acc.to(dtype=dtype, device="cpu")
            del acc
            torch.cuda.empty_cache()
    ...
    del distributed_deltas_cpu
    for i in range(len(distributed_deltas)):
        distributed_deltas[i] = None
    del distributed_deltas
    torch.cuda.empty_cache()
    merged_delta_flat_cpu = torch.zeros(total_params, dtype=dtype, device="cpu")

    for key, (start, end) in index_mgr.indices.items():
        if key in merged_per_key:
            merged_delta_flat_cpu[start:end] = merged_per_key[key].reshape(-1)

    del merged_per_key
    torch.cuda.empty_cache()

    merged_delta_flat_gpu = merged_delta_flat_cpu.to(main_device, non_blocking=True)
    del merged_delta_flat_cpu
    torch.cuda.empty_cache()

    print("[TSV-M] Finished TSV-based merging of task deltas.")
    return merged_delta_flat_gpu
...
def main():
    ...
    final_merged_delta_gpu = distributed_TSVM(
        distributed_deltas_r2,
        index_mgr=index_mgr,
    )
    ...
    # Final Addition & Save
    # ============================================================
    print(f"\n[Final] Combining Ortho Base + Scaling * TSVM Delta...")
